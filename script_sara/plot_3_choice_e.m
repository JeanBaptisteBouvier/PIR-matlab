function [] = plot_3_choice_e(choice, choice_help, sol, Lam1, Lam_t, r_f_syn, r_f , np, cr3bp, nro_T, nro_C, x0_T, x0_C, n_transfer, t_f,t_C, fg)
%% Variables
Lam  = Lam1;
...
r      = state_time(t_f(end),nro_T);
r_lvlh = syn2lvlh_e(t_f(end), r', r',  cr3bp.mu); % final position equal to target position: [0 0 0 0 0 0]'
w = r_lvlh(1);
e = r_lvlh(2);
t = r_lvlh(3);

% tc=Lam_t(1)-[0:0.05:0.5];
%  for i = 1:length(tc)
%    rT_c      = state_time(tc(i), nro_T);
%    rC_c      = state_time(tc(i)+t_C, nro_C);
%    yv_C_lvlh(1:6, i) = syn2lvlh(tc(i), rC_c', rT_c',  cr3bp.mu); 
%  end
 
 for j = 1:length(Lam)
   rT      = state_time(Lam_t(j), nro_T);
   rC      = state_time(Lam_t(j)- t_f(1) + t_C, nro_C);
   yv_C_lvlh(1:6, j) = syn2lvlh_e(Lam_t(j), rC', rT',  cr3bp.mu);
   
 end

 
 
 x0_C_lvlh = syn2lvlh_e(t_f(1), x0_C, x0_T,  cr3bp.mu); %Initial chaser position = initial point
...
for j = 1:length(Lam)
    Lam_lvlh(1:6, j) = rsyn2rlvlh_e(Lam_t(j), Lam(j,1:6)', Lam(j,7:12)',  cr3bp.mu);
end

%  Decomment the next line to visualize the effect of the first guess
% for r = 1:size(fg,1)
%     fg_lvlh(1:6, r) = rsyn2rlvlh_e(Lam_t(r), fg(r,1:6)', fg(r,7:12)',  cr3bp.mu);
% end
...
if np > 2
  for k = 2:np-1
       uu = state_time(t_f(k),nro_T);
       uu_lvlh = syn2lvlh_e(t_f(k), uu', uu', cr3bp.mu);
       a = uu_lvlh(1);
       b = uu_lvlh(2);
       c = uu_lvlh(3);
  ...
  %  Decomment the next two lines to visualize the effect of the first guess and comment
  %  the two lines after
  %  pp = [p1 p2 p3  p4 p7 p8 p9];
  %  nom = ['Chaser initial position' 'Chaser Orbit' 'RDV trajectory' str{:} 'Docking' 'First Guess' ' Approach Sphere'];
   pp = [p1 p2 p3  p4 p7 p9];
   nom = ['Chaser initial position' 'Chaser Orbit' 'RDV trajectory' str{:} 'Docking' ' Approach Sphere'];
   legend(pp,nom)
else
   ...
   legend([p1 p2 p3 p7 p9],'Chaser initial position', 'Chaser Orbit', 'RDV trajectory', 'Docking',' Approach Sphere')
end
