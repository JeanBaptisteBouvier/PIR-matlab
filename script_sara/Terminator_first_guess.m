function output = Terminator_first_guess(choice, LVLH, SYN, times_lin, times_cont, settings, nro_T, phi0, n_transf)
...
if choice(1)
   ...
   [Lam_t, Lam] = ode113(@(t,y)crtbp(t,y,cr3bp.mu),[tt T+tt],[x_CW_syn, state_time(tt, nro_T), phi0']', options); 
    sol.CW.Lam = Lam;
    sol.CW.Lam_t = Lam_t;
   %   Initial error     
...
if choice(2)
   ...
   [Lam_t, Lam] = ode113(@(t,y)crtbp(t,y,cr3bp.mu),[tt T+tt],[x_SL_syn, state_time(tt, nro_T), phi0']', options); 
    sol.SL.Lam = Lam;
    sol.SL.Lam_t = Lam_t;
  % Initial error      
...
if choice(3)
  ...
  [Lam_t, Lam] = ode113(@(t,y)crtbp(t,y,cr3bp.mu),[tt T+tt],[x_LR_syn, state_time(tt, nro_T), phi0']', options); 
   sol.LR.Lam = Lam;
   sol.LR.Lam_t = Lam_t;
