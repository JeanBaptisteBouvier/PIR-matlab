function output = Rendezvous_choice_e (choice, hold_points_dim, TOF_dim, init_cond, orbits, settings, continuation, plot_yes)
...
% states in LVLH
x0_LVLH     = rsyn2rlvlh_e(t, x0_rel, x0_T, cr3bp.mu);   %adim
...
if m_hp > 0
   for i=1:m_hp
       hp_syn(i,:) = rlvlh2rsyn_e(t_f(i+1), [hold_points(i,:) 0 0 0]', state_time(t_f(i+1),nro_T)', cr3bp.mu);
   end
...
for i = 1:np-1
    ...
    pop = Terminator_e(choice, LVLH, SYN, times_lin, times_cont, settings, nro_T, phi0, n_transf);
    sol{i} = pop;
end
...
for i = 1:np-1
    ...
    if choice(1) && choice(2) && choice(2) && ~choice_help
...
if plot_yes == 1
   for i = 1:np-1
      ...
      %plot_1   %linear models
       plot_3_choice_e(choice, choice_help, sol_plot, Lam_plot, Lam_plot_t, r_f_syn, r_f , np, cr3bp, nro_T, nro_C, x0_T, x0_C, i, t_f,t_C, fg_plot)  %Lambert
   end
end
