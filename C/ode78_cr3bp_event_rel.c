...
//-------------------------------------------------------------------------
//Custom
#include "ode78_cr3bp_event_rel.h"
                           ...
                           double *mu)                   //cr3bp mass ratio
{
    ...
    switch(nvar)
    {
        ...
        {
            mexPrintf("ode78_cr3bp_rel. Error: wrong number nvar of state variables. nvar must 12 or 48. return.");
            return;
        }
        ...
        int nrhs, const mxArray *prhs[])
{
    ...
    if(!(nlhs == 2 || nlhs == 4)) {
        mexErrMsgIdAndTxt("custom:ode78_cr3bp_event_rel:nlhs",
    ...
    //---------------------------------------------------------------------
    if(nts!=2) {
        mexErrMsgIdAndTxt("custom:ode78_bcp_rel:nts","The time vector (first input) must be of size 2: [t0 tf]");
    }
    
    if(nvar!=12 && nvar!=48)
    {
        mexPrintf("nvar = %d\n", nvar);
        mexErrMsgIdAndTxt("custom:ode78_bcp_rel:nts","The state vector (second input) must be of size either 12 or 48.");
    }
    ...
    if(NStructElems != 1)
    {
        mexErrMsgIdAndTxt("custom:ode78_cr3bp_event_rel:NStructElems",
    ...
    if(nfields != 7)
    {
        ...
        mexErrMsgIdAndTxt("custom:ode78_cr3bp_event_rel:nfields",
    ...
    if (strcmp(type, "FREE") == 0) {
        mexErrMsgIdAndTxt("custom:ode78_bcp_event_rel:type",
                "The event structure as type FREE. Use ode78_bcp_rel instead.");
    }
    ...
    {
        mexErrMsgIdAndTxt("custom:ode78_cr3bp_event_rel:maxEv","Number of required events must be < %d.", MAX_EVENTS);
    }
    ...
    // Integration
    //---------------------------------------------------------------------
    ode78_cr3bp_event_rel(&t, y, ye, te, y0, t0, tf, nvar, &val_par, &nEv, &mu);
    
    //print the number of events found
    //mexPrintf("ode78_cr3bp_event_rel. %d were found.\n", nEv);
    ...
    if(nlhs == 4)
    {
        ...
        //ode78_cr3bp_vec_rel(&t, y, tv, yv, nGrid, y0, t0, tf, nvar, &mu);
        ...
        do{
            ...
            // Integration
            //-------------------------------------------------------------
            nV = ode78_cr3bp_vec_var_rel(&t, y, tv, yv, nGrid, y0, t0, tf, nvar, &mu);
    ...
    if(nGrid >= 5e6)
    {
        mexErrMsgIdAndTxt("custom:ode78_cr3bp_event_rel:maxCapacity","Maximum capacity reached. Try with a smaller integration time.");
    }
