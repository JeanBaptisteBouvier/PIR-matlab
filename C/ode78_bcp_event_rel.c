/*=========================================================================
 * ode78_bcp_event_rel.c mex file to generate
                         ...
                         double *param)                //bcp parameters
{
    ...
    switch(nvar)
    {
        ...
        {
            mexPrintf("ode78_bcp. Error: wrong number nvar of state variables. nvar must 12 or 48. return.");
            return;
        }
                  ...
                  int nrhs, const mxArray *prhs[])
{
    ...
    //---------------------------------------------------------------------
    if(nrhs!=8) {
        mexErrMsgIdAndTxt("custom:ode78_bcp_event_rel:nrhs","8 inputs required.");
    }
    if(!(nlhs == 2 || nlhs == 4)) {
        mexErrMsgIdAndTxt("custom:ode78_bcp_event_rel:nlhs",
    ...
    // Integration
    //---------------------------------------------------------------------
    ode78_bcp_event_rel(&t, y, ye, te, y0, t0, tf, nvar, &val_par, &nEv, params);
     
    //print the number of events found
    //mexPrintf("ode78_bcp_event_rel. %d were found.\n", nEv);
    ...
    if(nlhs == 4)
    {
        ...
        do{
            ...
            // Integration
            //-------------------------------------------------------------
            nV = ode78_bcp_vec_var_rel(&t, y, tv, yv, nGrid, y0, t0, tf, nvar, params);
