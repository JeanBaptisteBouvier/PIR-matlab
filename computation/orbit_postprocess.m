...
switch(orbit.type)
    ...
    case cst.orbit.type.DRO
        orbit.Ax    = abs(orbit.y0(1)- (1-cr3bp.mu));
        orbit.Axdim = orbit.Ax*cr3bp.L;
end
...
%--------------------------------------------------------------------------
l = length(orbit.tv);
orbit.theta(1) = atan(yv(1,2)/(1-cr3bp.mu-yv(1,1))); % initialization
for k = 2:l
    y = yv(k,2);
    x = yv(k,1);
    orbit.theta(k) = atan(y/(1-cr3bp.mu-x));
    while (orbit.theta(k) < orbit.theta(k-1)) %to control the discontinuity of atan
        orbit.theta(k) = orbit.theta(k)+ pi;
    end
end
