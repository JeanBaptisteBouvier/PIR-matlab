...
figure(10)
p1 = plot(nro_T.yv(:,1),nro_T.yv(:,2),'g');  %Target 
hold on
axis equal
grid on
p2 = plot(nro_C.yv(:,1),nro_C.yv(:,2),'k');  %Chaser 
p3 = plot(x0_T(1), x0_T(2), 'mo');
p4 = plot(x0_C(1), x0_C(2), 'co','LineWidth',1.5); 
p5 = plot(Lam(:,7) + Lam(:,1), Lam(:,8) + Lam(:,2), 'r','LineWidth',1.5);
p7 = plot(w, e, 'ko','LineWidth',1.5);
