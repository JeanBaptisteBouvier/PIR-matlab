function output = Terminator(choice, LVLH, SYN, times_lin, times_cont, settings, nro_T, phi0, n_transf, primary)
...
if choice(1)        %% Clohessy Wiltshire (LVLH)
    [x_CW_LVLH, ~, ~] = Docking_CW(r_LVLH, v_LVLH, tt, tt_2, intervals, nro_T, cr3bp, primary);
    x_CW_syn = rlvlh2rsyn_c(tt, x_CW_LVLH', state_time(tt,nro_T)', cr3bp.mu)';
    x_syn = x_CW_syn;
...
if  choice(2)  %% Straight Line
    ...
    x_SL_syn  = rlvlh2rsyn_c(tt, x_SL_LVLH', state_time(tt,nro_T)', cr3bp.mu)';
    x_syn = x_SL_syn;
...
if  sum(choice) == 1
    [fg, Transfer, Lam_t, it, err] = Lambert_cycle(x_syn, r_next, state_time(tt, nro_T), phi0, T, toll, it_max, cr3bp, options, tt);
...
if choice(4) || choice_help 
   ...
   [fg, Transfer, Lam_t, it, err] = Continuation(T, TOF_cont, int_cont, r_syn, v_syn, r_next, tt, nro_T, intervals, phi0, toll, options, it_max, cr3bp); 
